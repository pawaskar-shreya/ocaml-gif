...
let _bits_from_string str =
  ...
  let rec next n =
    if n < len then
      LazyList.Cons
        ((int_of_char str.[n / 8] lsr (n mod 8)) land 1, fun () -> next (n + 1))
    else LazyList.Nil
  in
  next 0

(* z listy bitow wyciaga kod (int) o wskazanej dlugosci *)
let rec get_next_code input length =
...
let decode_bits input init_code_size =
  ...
  let rec decode input code_size avail_code prev_entry =
    ...
    if code = clear_code then
      ...
      LazyList.Cons
        ( [ next_code ],
          fun () -> decode tail' new_code_size (clear_code + 2) [ next_code ] )
    else if code = end_code then LazyList.Nil
    else
      let entry =
        if code < clear_code then [ code ]
        else if code < avail_code then dict.(code)
        else prev_entry @ [ List.hd prev_entry ]
      in
      dict.(avail_code) <- prev_entry @ [ List.hd entry ];
      let new_avail_code = avail_code + 1 in
      let new_code_size =
        if new_avail_code >= 1 lsl code_size then code_size + 1 else code_size
      in
      LazyList.Cons
        (entry, fun () -> decode tail new_code_size new_avail_code entry)
  in
  decode input (init_code_size + 1) (clear_code + 2) []

(* odpala dekompresje dla danego wejscia w postaci listy bajtow *)
let decode input code_size = decode_bits (bytes_to_bits input) code_size
        ...
        if word = [] then
          LazyList.Cons
            ( (clear_code, code_size),
              fun () ->
                encode (input_tail ()) dict [ char ] (init_code_size + 1)
                  (clear_code + 2) )
        else
          let word_char = char :: word in
          if EncDict.mem word_char dict then
            encode (input_tail ()) dict word_char code_size avail_code
          else
                ...
                ( (code, code_size),
                  fun () ->
                    LazyList.Cons
                      ( (clear_code, code_size),
                        fun () ->
                          encode (input_tail ()) EncDict.empty [ char ]
                            (init_code_size + 1) (clear_code + 2) ) )
            else
              LazyList.Cons
                ( (code, code_size),
                  fun () ->
                    encode (input_tail ()) new_dict [ char ] new_code_size
                      new_avail_code )
    | LazyList.Nil ->
        ...
        if word != [] then
          LazyList.Cons
            ((EncDict.find_word word dict, code_size), fun () -> ending)
        else ending
  in
...
let code_to_bits =
  let conv byte =
    [
      ...
      (byte land 256) lsr 8;
      (byte land 512) lsr 9;
      (byte land 1024) lsr 10;
      (byte land 2048) lsr 11;
    ]
  in
  let arr = Array.make 4096 [] in
  for i = 0 to 4095 do
    arr.(i) <- conv i
  done;
  let b2b byte = arr.(byte) in
...
let make_output codes_stream =
  let fn (code, size) = list_take size (code_to_bits code) in
...
let pack_codes_bits bits =
  let rec pack bits acc n =
    ...
    | LazyList.Cons (bit, tail) ->
        if n < 8 then pack (tail ()) (acc + (bit lsl n)) (n + 1)
        else LazyList.Cons (acc, fun () -> pack (tail ()) bit 1)
    | LazyList.Nil -> LazyList.Cons (acc, fun () -> LazyList.Nil)
  in
  pack bits 0 0
